...
async def emails_test(payload: EmailTestRequest, user: dict = Depends(get_current_user)):
    ...
    if t == "welcome":
        subject, html = render_welcome(name)
    elif t == "payment_success":
        subject, html = render_payment_success(name, "monthly", 9.99, "EUR")
    elif t == "trial_ending":
        subject, html = render_trial_ending(name, days_left=2)
        html = html.replace("{streak_workouts}", "4")
    elif t == "streak_reminder":
        subject, html = render_streak_reminder(name, days_since=4)
